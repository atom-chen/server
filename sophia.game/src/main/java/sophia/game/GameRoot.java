	...
	public static final State startUp() {
		...
		try {
			...
			if (Thread.currentThread().getContextClassLoader().getResource("") == null){
				...
				file = new File(Thread.currentThread().getContextClassLoader().getResource("").getFile()).getParentFile();
			}

			URL[] urls = ClazzUtil.getClassPathURLs(file, game_server_classes_path,
